...
class Index extends HomeBase
{
    ...
    public function index()
    {
        ...
        //发帖Top 12
        $tptm = Db::name('forum')->alias('f')->join('user u', 'f.uid=u.id')->field('u.*,count(f.id) as forumnum')->group('f.uid')->order('forumnum desc')->limit($this->site_config['b_home_phb'])->select();
        ...
        $settop['settop']   = 1;
        $nosettop['settop'] = 0;
        $forum_fields = '`f`.id,`f`.tid,`f`.uid,`f`.title,`f`.open,`f`.choice,`f`.settop,`f`.carousel,`f`.zan, `f`.view, `f`.time, `f`.add_time, `f`.reply, `f`.keywords, `f`.description, `f`.remarks, `f`.jiacu, `f`.yanse, `f`.memo, `f`.attach, `f`.coverpic, `f`.collect, `f`.attach/*, `f`.content*/';
        $tptc               = $forum->alias('f')->join('forumcate c', 'c.id=f.tid')->join('user m', 'm.id=f.uid', 'left')->field($forum_fields.',c.id as cid,m.id as userid,m.userhead,m.attestation,m.username,c.name')->where($open)->where($settop)->order('f.id desc')->limit($this->site_config['b_home_settop'])->select();
        $this->assign('tptc', $tptc);
        $tptcs = $forum->alias('f')->join('forumcate c', 'c.id=f.tid')->join('user m', 'm.id=f.uid', 'left')->field($forum_fields.',c.id as cid,m.id as userid,m.userhead,m.attestation,m.username,c.name')->where($open)->where($nosettop)->order('f.time desc')->paginate($this->site_config['b_home_main']);
    ...
    public function cate()
    {
        ...
        if (empty($cate)) {
            ...
            switch ($types) {
                case 'newreply':
                    $tptc = $forum->alias('f')->join('forumcate c', 'c.id=f.tid')->join('comment t', 'f.id=t.fid')->join('user m', 'm.id=t.uid')->field($forum_fields.',f.time as ftime,c.id as cid,t.time,m.id as userid,m.userhead,m.attestation,m.username,c.name')->where('f.tid', 'in', $children)->where('f.open', 1)->group('f.id')->order('t.time desc,f.id desc')->paginate($num);

                    break;
                case 'hot':
                    $tptc = $forum->alias('f')->join('forumcate c', 'c.id=f.tid')->join('user m', 'm.id=f.uid')->field($forum_fields.',c.id as cid,m.id as userid,m.userhead,m.attestation,m.username,c.name')->where('f.tid', 'in', $children)->where('f.open', 1)->order('f.reply desc,f.id desc')->paginate($num);

                    break;
                case 'choice':
                    // $choice['choice'] = 1;
                    $tptc = $forum->alias('f')->join('forumcate c', 'c.id=f.tid')->join('user m', 'm.id=f.uid')->field($forum_fields.',c.id as cid,m.id as userid,m.userhead,m.attestation,m.username,c.name')->where('f.tid', 'in', $children)->where('f.open', 1)->where('f.choice', 1)->order('f.settop desc,f.id desc')->paginate($num);
                    break;
                default:
                    $tptc = $forum->alias('f')->join('forumcate c', 'c.id=f.tid')->join('user m', 'm.id=f.uid')->field($forum_fields.',c.id as cid,c.alias as alias,m.id as userid,m.attestation,m.userhead,m.username,m.grades,c.name')->where('f.tid', 'in', $children)->where('f.open', 1)->order('f.settop desc,f.id desc')->paginate($num);
            }
    ...
    public function search()
    {
        ...
        if (empty($ks)) {
            ...
            $tptc = $forum->alias('f')->join('forumcate c', 'c.id=f.tid')->join('user m', 'm.id=f.uid')->field($forum_fields.',c.id as cid,m.id as userid,m.userhead,m.username,c.name')->order('f.id desc')->where($open)->where($map)->paginate(8, false, $config = ['query' => array('ks' => $ks)]);
