...
(function($) {
	...
	var pjax = function(options) {
		opt = pjax.options = $.extend(pjax.defaultOptions, options);
		if (!opt.container || !opt.selector) {
			throw new Error('selector & container options must be set');
		}
		$('body').delegate(opt.selector, 'click', function(event) {
			...
			if (typeof opt.filter === 'function') {
				if (opt.filter.call(this, href, this) === true){
					return true;
				}
			}
	...
	pjax.send = function(options,href,ele){
		var opt = pjax.options = $.extend(pjax.defaultOptions, options);		
		// 只是hash不同
	...
	// 发送请求
	pjax.request = function(opt) {
		var opt = pjax.options;
		/*if(options.hasOwnProperty('data')){
			pjax.defaultOptions.data=options.data;
		}*

		opt = $.extend(true, pjax.defaultOptions, options);*/
		var cache, container = $(opt.container);
		opt.oldUrl = opt.url;
		opt.url = Util.getRealUrl(opt.url);
		if($(opt.element).length){
			cache = Util.toInt($(opt.element).attr('data-pjax-cache'));
			if (cache) {
				opt.cache = cache;
			}
		}
		...
		opt.cache = Util.toInt(opt.cache);
		...
		// 展现函数
		if (!opt.showFn) {
			opt.showFn = function(data, fn, isCached) {
				pjax.showFn(opt.show, container, data, fn, isCached);
			};
		}
		pjax.options = opt;
		//console.log(pjax.options.callback,opt);
		pjax.options.success = pjax.success;
		if (opt.cache && (cache = Util.getCache(opt.url, opt.cache, opt.storage))) {
			opt.beforeSend();
			opt.title = cache.title;
	...
	if (!Util.support.pjax) {
		...
		pjax.request = function(opt) {
			if (pjax.options && pjax.options.url) {
