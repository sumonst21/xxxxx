        ...
        layui.each(n, function (a, n) {
            var l = n.children && n.children.length > 0,
                ...
                s = o(["<li " + (n.spread ? 'data-spread="' + n.spread + '"' : "") + ">", function () {
                        return l ? '<i class="layui-icon layui-tree-spread">' + (n.spread ? t.arrow[1] : t.arrow[0]) +
                            "</i>" : ""
                    }(), function () {
                        return r.check ? '<i class="layui-icon layui-tree-check">' + ("checkbox" === r.check ? t.checkbox[
                            0] : "radio" === r.check ? t.radio[0] : "") + "</i>" : ""
                    }(), function () {
                        return '<a href="' + (n.href || "javascript:;") + '" ' + (r.target && n.href ? 'target="' + r.target +
                            '"' : "") + ">" + ("<input type=\"checkbox\" title=\"" + (n.name || "未命名") +
                            "\" lay-skin=\"primary\"name=\"rules[]\" value=\"" + n.id + "\"></a>")
                    }(), "</li>"].join(""));
