...
class Member extends Base
{
    ...
    public function login() {
		if(\think\Session::has("uid") && \think\Session("uid")>0) {
            ...
            if(request()->isPost()){
                ...
                if(false === $vo) {
                    ...
                    //\think\Cookie::set('loginfo',authcode($vo['id'].','.$vo['nickname'].','.$vo['account'],'ENCODE'));
                    \think\Session::set(Config('USER_AUTH_KEY'),$vo['id']);
                    \think\Session::set('nickname',$vo['nickname']);
                    if($vo['id']==261) {
                        \think\Session::set(Config('ADMIN_AUTH_KEY'),true);
                    }
                    //保存登录信息
                    $time	=	time();
                    $data = array();
